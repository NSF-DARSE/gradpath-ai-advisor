...
def build_full_graduation_plan(
    ...
    elective_course_ids: Optional[List[str]] = None,
) -> List[Dict[str, Any]]:
    ...
    # Append user-requested electives (only if in catalog and not already required)
    elective_normalized: List[str] = []
    if elective_course_ids:
        req_set = set(required_normalized) | set(blocker_prereqs)
        for cid in elective_course_ids:
            n = _normalize(cid)
            if n in catalog_lookup and n not in req_set:
                elective_normalized.append(n)
    # Collect prerequisites for electives that aren't already in the plan
    elective_blocker_prereqs = _collect_all_prerequisites(elective_normalized, catalog_lookup)
    already_in_plan = set(all_to_plan)
    extra_prereqs = [p for p in elective_blocker_prereqs if p not in already_in_plan]
    all_to_plan = all_to_plan + extra_prereqs + elective_normalized
    ...
    elective_set = set(elective_normalized)
    unplanned_required = [c for c in remaining if c not in elective_set]
    unplanned_electives = [c for c in remaining if c in elective_set]

    return {
        "planned": planned,
        "unplanned": unplanned_required,
        "unplanned_electives": unplanned_electives,
